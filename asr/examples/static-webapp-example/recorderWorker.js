...
function init(config){
  ...
  console.log("[DEBUG] Recording sample rate "  + sampleRate);
...
function export16kMono(type){
  var bigbuffer = mergeBuffers(recBuffers, recLength);
  var buffer = bigbuffer.subarray(pointer);
  console.log("[DEBUG] " + bigbuffer.length + " " + buffer.length + "  " + pointer);
  pointer += buffer.length;

  // var samples = resampler.resampler(buffer);
  var dataview = encodeRAW(buffer);
  var audioBlob = new Blob([dataview], { type: type });
  // console.log("[DEBUG] AUDIO SIZE " + buffer.length + "  " + audioBlob.size);
  this.postMessage(audioBlob);
}
